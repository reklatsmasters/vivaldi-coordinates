...
module.exports = class VivaldiPosition {
	...
	update(rtt, cj, ej) {
		...
		if (cj instanceof VivaldiPosition) {
			let coords = cj.getCoordinates();
			return this.update(rtt, coords, coords.getErrorEstimate());
		}
		
		if (!valid(rtt) || !valid(cj) || !valid(ej)) {
	...
	/**
	 * @param {HeightCoordinates|VivaldiPosition} coords
	 * @returns {number}
	 */
	estimateRTT(data) {
